...
export const CATALOG = [
  ...
  { route: "GET /v1/proxy/port/30d", price: "$29.00", desc: "DEDICATED MOBILE PORT, 30 DAYS — your own entry on a real 4G/5G carrier IP (Orange, AS16028), 10 GB included over 30 days. Cheaper per GB than the metered bundles: the market standard for mobile proxies is a port for a period, not a byte counter. Carrier/ASN/uptime verified live and returned with the key. Fact sheet: /proxy",
    bazaar: { method: "GET", input: {}, output: { example: { key: "rp1.10.1790000000.xxxx", gb: 10, valid_days: 30, tier: "mobile" } } } },
  { route: "GET /v1/proxy/port/7d", price: "$9.00", desc: "MOBILE PORT, 7-DAY TEST — 3 GB over 7 days on a real mobile-carrier IP (Orange, AS16028). Made to qualify the exit before committing to a month. Carrier/ASN/uptime returned with the key. Fact sheet: /proxy",
    bazaar: { method: "GET", input: {}, output: { example: { key: "rp1.3.1790000000.xxxx", gb: 3, valid_days: 7, tier: "mobile" } } } },
